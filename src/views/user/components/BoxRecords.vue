...
import {getMyBoxRecordApi} from "@/api";
import {ref} from "vue";
...
const getList = () => {
  loading.value = true
  getMyBoxRecordApi(form.value).then(res => {
...
<template>
  <div class="records">
    <div class="records-item">
      <div class="item name">箱子名称</div>
      <div class="item time">时间</div>
      <div class="item weapon">饰品</div>
    </div>
    <el-scrollbar height="400px" @scroll="onScroll" ref="scrollRef" v-loading="loading">
      <div class="records-list" ref="listRef">
        <div class="records-item" v-for="(i,index) in list" :key="index">
          <div class="item name">{{i.boxName}}</div>
          <div class="item time">{{i.updateTime}}</div>
